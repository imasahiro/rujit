...
/* original code was based on ./vm_args.c */
...
#define CALLER_SETUP_ARG(cfp, ci)                    \
    do {                                             \
	if (UNLIKELY(IS_ARGS_SPLAT(ci)))             \
	    jit_caller_setup_arg_splat((cfp), (ci)); \
	if (UNLIKELY(IS_ARGS_KEYWORD(ci)))           \
	    jit_caller_setup_arg_kw((cfp), (ci));    \
    } while (0)

static inline void
jit_caller_setup_arg_splat(rb_control_frame_t *cfp, rb_call_info_t *ci)
{
...
static inline void
jit_caller_setup_arg_kw(rb_control_frame_t *cfp, rb_call_info_t *ci)
{
