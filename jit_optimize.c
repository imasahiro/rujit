...
static void lir_inst_replace_with(trace_recorder_t *rec, lir_inst_t *inst, lir_inst_t *newinst)
{
	...
	for (i = 0; i < inst->user->size; i++) {
		...
		if (ref && *ref == inst) {
		    *ref = newinst;
		    lir_inst_adduser(rec, newinst, user);
		}
		j += 1;
	    }
	}
...
static void worklist_init2(worklist_t *list, jit_list_t *blocks, trace_recorder_t *rec)
{
    unsigned i;
    jit_list_init(&list->list);
    list->rec = rec;
...
static int apply_bb_worklist(trace_recorder_t *rec, worklist_bb_func_t func)
{
    int modified = 0;
    worklist_t worklist;
...
static int apply_worklist(trace_recorder_t *rec, worklist_ir_func_t func)
{
    int modified = 0;
    worklist_t worklist;
...
static void remove_from_side_exit(trace_recorder_t *rec, lir_inst_t *inst)
{
    unsigned j, k;
    for (k = 0; k < rec->bblist.size; k++) {
	basicblock_t *bb = (basicblock_t *)jit_list_get(&rec->bblist, k);
...
static void remove_side_exit(trace_recorder_t *rec, lir_inst_t *inst)
{
    basicblock_t *bb = inst->parent;
    VALUE *pc = ((IGuardTypeFixnum *)inst)->Exit;
    int idx = jit_list_indexof(&bb->stack_map, (uintptr_t)pc);
...
static int copy_propagation(worklist_t *list, basicblock_t *bb)
{
	...
	if (inst->opcode == OPCODE_ILoadSelf) {
		...
		if (inst2->opcode == OPCODE_ILoadSelf) {
		    lir_inst_replace_with(rec, inst2, inst);
	...
	if (inst->opcode == OPCODE_IInvokeMethod) {
		...
		for (j = i + 1; j < bb->insts.size; j++) {
			...
			if (ir2->Level == 0 && ir2->Index == index) {
			    worklist_push(&worklist, inst2);
			    lir_inst_replace_with(rec, inst2, argN);
			    break;
			}
		    }
		    if (inst2->opcode == OPCODE_IEnvStore) {
...
static int dead_store_elimination(worklist_t *list, basicblock_t *bb)
{
	...
	if (inst->opcode == OPCODE_IEnvStore) {
		...
		if (inst->opcode == inst2->opcode) {
		    ...
		    IEnvStore *ir2 = (IEnvStore *)inst2;
		    if (ir1->Level == ir2->Level && ir1->Index == ir2->Index) {
			lir_inst_replace_with(rec, inst2, ir2->Val);
			worklist_push(&worklist, inst2);
		    }
		}
	    }
...
static lir_t trace_recorder_create_undef(trace_recorder_t *rec, basicblock_t *bb)
{
    lir_t reg;
    basicblock_t *prev = rec->cur_bb;
...
static lir_t trace_recorder_create_load(trace_recorder_t *rec, basicblock_t *bb, int idx, int lev)
{
    lir_t reg;
    basicblock_t *prev = rec->cur_bb;
    unsigned inst_size = bb->insts.size;
...
static lir_t trace_recorder_create_phi(trace_recorder_t *rec, basicblock_t *bb, int argc, lir_t *argv)
{
    lir_t reg;
    basicblock_t *prev = rec->cur_bb;
    unsigned inst_size = bb->insts.size;
    rec->cur_bb = bb;
    reg = Emit_Phi(rec, argc, argv);
    if (inst_size > 0 && inst_size != basicblock_size(bb)) {
...
static void trace_recorder_replace_phi_argument(trace_recorder_t *rec, IPhi *phi, int idx, lir_t reg)
{
    lir_inst_removeuser(phi->argv[idx], (lir_t)phi);
    lir_inst_adduser(rec, reg, (lir_t)phi);
    phi->argv[idx] = reg;
}
...
static int insert_phi(worklist_t *list, basicblock_t *bb)
{
	...
	while (variable_table_each(&itr)) {
		...
		lir_t phi;
		lir_t undef = trace_recorder_create_undef(rec, bb);
		lir_t argv[bb->preds.size];
		for (j = 0; j < bb->preds.size; j++) {
		    argv[j] = undef;
		}
		phi = trace_recorder_create_phi(rec, bb, bb->preds.size, argv);
	...
	while (variable_table_each(&itr)) {
	    ...
	    if (reg == NULL) {
		reg = trace_recorder_create_load(rec, pred, itr.idx, itr.lev);
	    }
	    trace_recorder_replace_phi_argument(rec, phi, i, reg);
	}
    }
    bb->init_table = newvt;
    return 0;
...
static void loop_invariant_code_motion(trace_recorder_t *rec, int move_block)
{
    unsigned i, j, k;
    lir_t *ref = NULL;
    for (j = 0; j < rec->bblist.size; j++) {
	basicblock_t *bb = (basicblock_t *)jit_list_get(&rec->bblist, j);
	for (i = 0; i < bb->insts.size; i++) {
	    lir_inst_t *inst = basicblock_get(bb, i);
...
static void trace_optimize(trace_recorder_t *rec, trace_t *trace)
{
    ...
    apply_bb_worklist(rec, check_side_exit_reference_count);
    apply_bb_worklist(rec, check_basicblock_link);
    apply_bb_worklist(rec, check_basicblock_flag);
    apply_bb_worklist(rec, remove_duplicated_guard);
    while (modified) {
	modified = 0;
	...
	if (modified) {
	    modified += apply_bb_worklist(rec, instruction_aggregation);
	    modified += apply_bb_worklist(rec, remove_duplicated_guard);
	}
    }
    if (LIR_OPT_LOOP_INVARIANT_CODE_MOTION) {
	apply_bb_worklist(rec, insert_phi);
	loop_invariant_code_motion(rec, LIR_OPT_BLOCK_GUARD_MOTION);
    }
